...
export const RegisterPage = () => {
  ...
  async function checkForm() {
    ...
    else if (form.password.length < 6) setValidError('Пароль должен быть больше 6-ти символов!')
    else if (form.password !== form.confirmPassword) setValidError('Пароли не совпадают!')
    else {
      setValidError('')
      try {
  ...
  async function validInfo() {
    ...
    else if (info.height === '') setValidError('Поле "Рост" не может быть пустым!')
    else if (info.weight === '') setValidError('Поле "Вес" не может быть пустым!')
    else {
      setValidError('')
      let date = info.birth.split('.')
      let reqObject = {
            ...
            <div className='container'>
              ...
              <div>
                {activities.map((item, index) => {
                  return <ActivityCard key={index} item={item} active={item.name} />
                })}
              </div>
          ...
          <div>
            ...
            <div className='container'>
              ...
              <div className={profileModule.profile__group}>
                ...
                <input type='checkbox' id='calcCheck' className={profileModule.profile__inputCheck} checked={isChecked} onChange={() => setIsChecked(!isChecked)} />
              </div>
              ...
              <div className={profileModule.profile__group}>
                ...
                <div className={profileModule.gender__wrapper}>
                  <div className={`${profileModule.gender__item} ${genderIsMale && profileModule.active}`} onClick={() => setGenderIsMale(true)}>
                    Муж.
                  </div>
                  <div className={`${profileModule.gender__item} ${!genderIsMale && profileModule.active}`} onClick={() => setGenderIsMale(false)}>
                    Жен.
                  </div>
                </div>
              </div>
